...
export function DepositModal({ isOpen, onClose }: DepositModalProps) {
  ...
  const { createDeposit, sendViaTonConnect, depositInfo, error, reset } = useDeposit();
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      // Try TonConnect auto-send if wallet is connected
      if (isWalletConnected && deposit) {
        await sendViaTonConnect(deposit);
      }
    } catch {
  ...
  const handleTonConnectSend = async () => {
    if (!depositInfo) return;
    hapticFeedback?.('medium');
    await sendViaTonConnect(depositInfo);
  };
  ...
  const renderContent = () => {
    // Status screens: confirming / completed / failed
    ...
    // Awaiting transaction — show address + memo + TonConnect button
    if (depositStatus === 'awaiting_tx' && depositInfo) {
      return (
        <div className="space-y-4">
          <div className="text-center mb-2">
            <p className="text-2xl font-bold text-tg-text">{depositInfo.amount} TON</p>
          </div>

          {/* Quick pay button if wallet connected */}
          {isWalletConnected && (
            <Button variant="primary" fullWidth onClick={handleTonConnectSend}>
              <Wallet size={18} />
              {t.wallet.payNow}
            </Button>
          )}
          ...
          <div className="p-3 rounded-tg bg-tg-bg-secondary">
            <p className="text-xs text-tg-text-secondary mb-1">Wallet address</p>
            <div className="flex items-center gap-2">
              <code className="text-xs font-mono text-tg-text flex-1 break-all">
                {depositInfo.address}
              </code>
              <button
                onClick={() => copyToClipboard(depositInfo.address, 'address')}
                className="p-1.5 rounded-lg bg-tg-bg hover:bg-tg-bg-secondary shrink-0"
              >
                {copiedField === 'address' ? <Check size={14} className="text-tg-success" /> : <Copy size={14} className="text-tg-text-secondary" />}
              </button>
            </div>
          </div>

          {/* Memo */}
          <div className="p-3 rounded-tg bg-tg-bg-secondary border border-tg-warning/30">
            <p className="text-xs text-tg-warning font-medium mb-1">Memo (required!)</p>
            <div className="flex items-center gap-2">
              <code className="text-sm font-mono text-tg-text flex-1 break-all font-bold">
                {depositInfo.memo}
              </code>
              <button
                ...
                className="p-1.5 rounded-lg bg-tg-bg hover:bg-tg-bg-secondary shrink-0"
              >
                {copiedField === 'memo' ? <Check size={14} className="text-tg-success" /> : <Copy size={14} className="text-tg-text-secondary" />}
              </button>
            </div>
          </div>

          <p className="text-xs text-tg-error font-medium text-center">
            Without the memo your deposit cannot be identified!
          </p>

          {/* Polling indicator */}
          <div className="flex items-center justify-center gap-2 text-xs text-tg-text-secondary">
