...
function extractPublicKey(stateInit: ParsedStateInit): Buffer {
  ...
  // Try wallet v5r1 format: is_signature_auth_allowed(1) + seqno(32) + wallet_id(32) + pubkey(256)
  try {
    const dataSlice = stateInit.data.beginParse();
    dataSlice.loadBit(); // is_signature_auth_allowed
    dataSlice.loadUint(32); // seqno
    dataSlice.loadInt(32); // wallet_id
